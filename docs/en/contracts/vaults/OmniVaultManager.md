...
Internal function to check for duplicate token IDs in a deposit request

**Dev notes:** \
This is an O(n^2) operation but the number of tokens per vault is expected to be between 2-5

```solidity:no-line-numbers
function _checkDuplicateTokenIds(uint16[] _tokenIds) internal pure
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| _tokenIds | uint16[] | The array of token IDs to check |
