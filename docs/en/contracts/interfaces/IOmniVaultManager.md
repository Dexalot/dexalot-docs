...
```solidity:no-line-numbers
function vaultDetails(uint256 _vaultId) external view returns (string name, address proposer, address omniTrader, enum IOmniVaultManager.VaultStatus status, address executor, address shareToken, address dexalotRFQ)
```
