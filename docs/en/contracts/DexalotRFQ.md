...
| TRUSTED_FORWARDER_ROLE | bytes32 |
| VERSION | bytes32 |
...
```solidity:no-line-numbers
function _verifySwapInternal(uint256 _nonceAndMeta, uint256 _expiry, address _taker, address _sender, bool _isAggregator, bytes32 _hashedStruct, bytes _signature) private
```
