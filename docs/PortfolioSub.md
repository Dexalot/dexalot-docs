...
_Initializes with the native deposit threshold, users can deposit ALOT if they at least have 0.05 ALOT._
...
_Only callable by the default admin_
...
_WHEN Increasing in addExectuion the amount is applied to both total and available
...
_DEPOSIT messages are the only message that can be sent to the portfolio sub for the moment
                Even when the contract is paused, this method is allowed for the messages that
                are in flight to complete properly.
                CAUTION: if Paused for upgrade, wait to make sure no messages are in fligh then upgrade._
...
_Only call this just before calling force resume receive function for the LZ bridge
...
_This function decreases ALOT balance of the user and calls the PortfolioMinter to mint the native ALOT_
...
_This is not a ERC20 transfer, this is a balance transfer between portfolios_
...
_Calculates the swap amount for each token for the given gas amount_
...
_This threshold checks the users remaining native balance while depositing native from subnet wallet._
...
_Only callable by admin
    We don't allow tokens with the same symbols.
...
| _srcChainId | uint32 | Source Chain id, overwritten by srcChain of Portolio.           it will only be used by PortfolioBridgeSub |
...
_tokenTotals for the symbol should be 0 before it can be removed
