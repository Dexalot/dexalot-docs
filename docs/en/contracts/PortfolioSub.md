...
Calculates the maker and taker fees and the collector's cut of the taker fee

**Dev notes:** \
The cut is a capped rate, so it is resolved here where the notional is still in scope.
collectorRate is min(cap, takerRate), which keeps takerFeeCollectorAmount <= takerFee.

```solidity:no-line-numbers
function calculateFeeAmounts(struct ITradePairs.Execution _execution) private view returns (uint256 makerFee, uint256 takerFee, address takerFeeCollector, uint256 takerFeeCollectorAmount)
```
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| makerFee | uint256 | Fee charged to the maker |
| takerFee | uint256 | Fee charged to the taker |
| takerFeeCollector | address | Maker earning part of the taker fee, or address(0) |
| takerFeeCollectorAmount | uint256 | Portion of takerFee that goes to takerFeeCollector |
...
```solidity:no-line-numbers
function safeIncrease(address _trader, bytes32 _symbol, uint256 _amount, uint256 _feeCharged, enum IPortfolio.Tx _transaction, address _traderOther, address _feeAddress, uint256 _feeAddressAmount) private
```
...
| _feeAddress | address | Address to receive a share of the fee, if feeCharged > 0 |
| _feeAddressAmount | uint256 | Portion of `_feeCharged` for `_feeAddress`, the rest goes to feeAddress. 0 sends the entire fee to feeAddress. |
...
```solidity:no-line-numbers
function transferToken(address _from, address _to, bytes32 _symbol, uint256 _quantity, uint256 _feeCharged, enum IPortfolio.Tx _transaction, bool _decreaseTotalOnly, address _feeAddress, uint256 _feeAddressAmount) private
```
...
| _feeAddress | address | Address to receive a share of the fee, if feeCharged > 0 |
| _feeAddressAmount | uint256 | Portion of the fee `_feeAddress` receives. See safeIncrease. |

#### auctionTransferAllowed

Whether `_sender` may transfer `_symbol` between L1 accounts

**Dev notes:** \
Auction tokens are frozen for account to account transfers so holders can't settle
around the auction's price discovery. Accounts with AUCTION_TRANSFER_ROLE are exempt, as fees
accrue in whatever token was traded and the fee collector must still be able to close an epoch.

```solidity:no-line-numbers
function auctionTransferAllowed(bytes32 _symbol, address _sender) private view returns (bool)
```
...
| ---- | ---- | ----------- |
| _symbol | bytes32 | Symbol of the token |
| _sender | address | Address initiating the transfer |

##### Return values

| Name | Type | Description |
| ---- | ---- | ----------- |
| [0] | bool | bool  True if the transfer is allowed |
