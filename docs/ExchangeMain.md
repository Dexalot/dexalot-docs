...
```solidity
function flipCoin() external nonpayable
```
...
| Name | Type | Description |
|---|---|---|
| _0 | contract AggregatorV3Interface | AggregatorV3Interface  price feed contract |
...
```solidity
function isHead() external view returns (uint80 r, int256 p, bool o)
```
...
```solidity
function setPriceFeed(address _address) external nonpayable
```
