...
_tokenMap balance for the symbol should be 0 before it can be removed.
...
_Used by Avalaunch for DD deposits and Vesting Contracts.
...
_Only process WITHDRAW messages as it is the only message that can be sent to the portfolio main
...
_Collect fees to pay for the bridge as native token
    Only the owner can call this function_
